...
| `DEV-1` 领域模型 + 权限 + 数据模型 | B 已冻结给 C/D 的第一批上游契约：`works` 表、`WorkDAO` 签名和 D 可依赖的 `RaceProjectDAO` 边界；后续继续补完整 Race / Work Service。 | `docs/b-upstream-contracts-for-cd.md`、`backend/tests/test_b_upstream_contracts.py` |
