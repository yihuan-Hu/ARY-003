...
# 旧 app.py 的 create_app（使用旧 routes/services/daos/schema）
# 注意：from app import create_app 现在会从 app/ 包导入（新 app），
# 所以必须通过文件路径导入旧 app.py。
_old_app_path = os.path.join(os.path.dirname(__file__), '..', '..', 'app.py')
import importlib.util
_spec = importlib.util.spec_from_file_location("old_app", os.path.abspath(_old_app_path))
_old_app_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_old_app_module)
create_app = _old_app_module.create_app
